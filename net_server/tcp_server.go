...
func (srv *TcpServer) handleListen() {
	for {
		...
		if err != nil {
			slog.Errorf("handleListen():error in accpt(): %s", err.Error())
...
func (srv *TcpServer) tcpRecver(conn net.Conn, tcpConn TcpConn) {
	...
	for {
		...
		if err != nil {
			slog.Warn("tcpRecver():err in ReadHeader():", err.Error())
			break
		}
		...
		if header.MsgType != MSG_TYPE_REQUEST &&
			...
			slog.Warnf("tcpRecver():error: msg type is not request[%d]",
				header.MsgType)
		...
		if bodyLen <= 0 {
			slog.Warnf("tcpRecver():err: bodyLen = %d", bodyLen)
		...
		if bodyLen > RECV_BUF_LEN {
			...
			if err != nil {
				slog.Warnf("tcpRecver():err in ReadBypass():", err.Error())
		...
		if err != nil {
			slog.Warnf("tcpRecver():err in readBody():", err.Error())
			break
		}
...
func (srv *TcpServer) tcpSender(conn net.Conn, tcpConn TcpConn) {
	...
	for {
		...
		if sendMsg.cmd != SEND_CMD_SEND {
			slog.Warnf("cmd should be SEND_CMD_SEND, now:%d", sendMsg.cmd)
			continue
		}
		...
		if err != nil {
			slog.Warnf("tcpSender():err in SendMsgMake():%s", err.Error())
		...
		if err != nil {
			slog.Warnf("tcpSender:err in WriteMsg():", err.Error())
		...
		slog.Debug("tcpSender:succ in write()")
		srv.tcpState.Inc("RESPONSE_SEND_OK", 1)
	}
	slog.Info("tcpSender quit")
}
